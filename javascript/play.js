...
for (let step1 = 0; step1 < 30; step1++) {
    for (let step2 = 0; step2 < 20; step2++) {
        ...
        if (btnMap[step1][step2] == "grw"){
            ...
            redButtons[step1+"-"+step2] =  [0, parseInt(btnMap[step1][step2].split("-")[1])];
            ...
            btnId.style.transform = "scale(1)";
            btnId.innerHTML = btnMap[step1][step2].split("-")[1]
            greenButtons[step1+"-"+step2] =  [0, parseInt(btnMap[step1][step2].split("-")[1])];
            ...
            btnId.style.transform = "scale(1)";
            btnId.innerHTML = btnMap[step1][step2].split("-")[1]
            blueButtons[step1+"-"+step2] =  [0, parseInt(btnMap[step1][step2].split("-")[1])];
...
var validBlock = ["b", "rb", "gb", "bb", "grg", "rg_down", "gg_down", "bg_down", "rb_pressed", "bb_pressed", "gb_pressed"];
...
document.addEventListener('keydown', (event) => {
    ...
    if (name == "d"){
        ...
        if (validBlock.indexOf(btnMap[playerPosion[0]+1][playerPosion[1]].split("-")[0]) > -1){
                playerPosion[0]++;
                document.getElementById("playerBtn").style.left = playerPosion[0]*4 + "vh";
        ...
        if (validBlock.indexOf(btnMap[playerPosion[0]][playerPosion[1]+1].split("-")[0]) > -1){
                playerPosion[1]++;
                document.getElementById("playerBtn").style.top = playerPosion[1]*4 + "vh";
        }
    }else if (name == "w"){
    ...
    if (name == "w" || name == "a" ||name == "s" ||name == "d") {
        if (btnMap[playerPosion[0]][playerPosion[1]] == "grg") {
            ...
            btnId = document.getElementById("square-"+playerPosion[0]+"-"+playerPosion[1]);
            btnId.style.backgroundColor = "Lightgrey";
            btnId.style.border = "3px solid rgb(120,120,120)";
            btnId.style.borderRadius = "10px";
            btnId.style.transform = "scale(1)";
        ...
        if (btnMap[playerPosion[0]][playerPosion[1]].split("-")[0] == "rb") {
            ...
            console.log("red button value:", btnMap[playerPosion[0]][playerPosion[1]].split("-"));
            var value =  btnMap[playerPosion[0]][playerPosion[1]].split("-")[1];
            btnMap[playerPosion[0]][playerPosion[1]] = "rb_pressed-"+value;
            console.log(btnMap[playerPosion[0]][playerPosion[1]]);
            console.log("redButtons",redButtons);
            console.log("player coords",playerPosion[0]+"-"+playerPosion[1])
            redButtons[playerPosion[0]+"-"+playerPosion[1]][0] = value;
            if (redButtons[playerPosion[0]+"-"+playerPosion[1]][0] > redMovesTotal){
                redMovesTotal = redButtons[playerPosion[0]+"-"+playerPosion[1]][0];
                console.log("button is larger")
            }
            console.log(redMovesTotal)
        }
        if (btnMap[playerPosion[0]][playerPosion[1]].split("-")[0] == "gb") {
            ...
            btnId.style.borderRadius = "50%";
            btnId.style.transform = "scale(0.9)";
            console.log("green button value:", btnMap[playerPosion[0]][playerPosion[1]].split("-"));
            var value =  btnMap[playerPosion[0]][playerPosion[1]].split("-")[1];
            ...
            if (greenButtons[playerPosion[0]+"-"+playerPosion[1]][0] > greenMovesTotal){
                greenMovesTotal = greenButtons[playerPosion[0]+"-"+playerPosion[1]][0];
                console.log("button is larger")
            }
            console.log(greenMovesTotal)
        }
        if (btnMap[playerPosion[0]][playerPosion[1]].split("-")[0] == "bb") {
            ...
            blueButtons[playerPosion[0]+"-"+playerPosion[1]][0] = blueButtons[playerPosion[0]+"-"+playerPosion[1]][1];
            if (blueButtons[playerPosion[0]+"-"+playerPosion[1]][0] > blueMovesTotal){
                blueMovesTotal = blueButtons[playerPosion[0]+"-"+playerPosion[1]][0];
                console.log("button is larger")
            }
            console.log(blueMovesTotal)
        }
        update();
    }
}, false);
    ...
    for (let step1 = 0; step1 < 30; step1++) {
        ...
        try{
            if (redButtons[step1+"-"+step2][0] != 0){
                redButtons[step1+"-"+step2][0] = redButtons[step1+"-"+step2][0] - 1;
            }
            console.log(greenButtons[step1+"-"+step2][0])
            

        }catch(e){}
        try{
            if (greenButtons[step1+"-"+step2][0] != 0){
                greenButtons[step1+"-"+step2][0] = greenButtons[step1+"-"+step2][0] - 1;
                console.log("green btn getimg less")
            }
        }catch(e){}

        try{
            if (blueButtons[step1+"-"+step2][0] != 0){
                blueButtons[step1+"-"+step2][0] = blueButtons[step1+"-"+step2][0] - 1;
                console.log("blue btn getimg less")
            }
        }catch(e){}

        try {
            if (btnMap[step1][step2].split("-")[0] == "rb_pressed" & redButtons[step1+"-"+step2][0] == 0){
                btnId = document.getElementById("square-"+step1+"-"+step2);
                redButtons[step1+"-"+step2][0] == redButtons[step1+"-"+step2][1];
                btnId.style.transform = "scale(1)";
        ...
        try{
        if (btnMap[step1][step2].split("-")[0] == "gb_pressed" & greenButtons[step1+"-"+step2][0] == 0){
                btnId = document.getElementById("square-"+step1+"-"+step2);
                greenButtons[step1+"-"+step2][0] == greenButtons[step1+"-"+step2][1];
                btnId.style.transform = "scale(1)";
        ...
        try{
            if (btnMap[step1][step2].split("-")[0] == "bb_pressed" & blueButtons[step1+"-"+step2][0] == 0){
                    btnId = document.getElementById("square-"+step1+"-"+step2);
                    blueButtons[step1+"-"+step2][0] == blueButtons[step1+"-"+step2][1];
                    btnId.style.transform = "scale(1)";
        ...
        if (btnMap[step1][step2] == "rg" & redMovesTotal > 0){
                ...
                console.log("red wall down");
        }
        if (btnMap[step1][step2] == "gg" & greenMovesTotal > 0){
            btnId = document.getElementById("square-"+step1+"-"+step2);
            btnId.style.transform = "scale(0.5)";
            btnMap[step1][step2] = "gg_down";
            console.log(" green wall down");
        }
        if (btnMap[step1][step2] == "bg" & blueMovesTotal > 0){
        btnId = document.getElementById("square-"+step1+"-"+step2);
        btnId.style.transform = "scale(0.5)";
        btnMap[step1][step2] = "bg_down";
        console.log(" blue wall down");
        }
        if (btnMap[step1][step2].split("-")[0] == "rb_pressed"){ //seting the contense of the button
            console.log("red buttons",redButtons);
            console.log("steps:",step1,step2);
            var num = redButtons[step1 + "-" + step2][0];
            document.getElementById("square-"+step1+"-"+step2).innerHTML = num;
        }
        if (btnMap[step1][step2].split("-")[0] == "gb_pressed"){ //seting the contense of the button
            var numgreen = greenButtons[step1 + "-" + step2][0];
            document.getElementById("square-"+step1+"-"+step2).innerHTML = numgreen;
            
        }
        if (btnMap[step1][step2].split("-")[0] == "bb_pressed"){ //seting the contense of the button
            console.log(blueButtons)
            var numblue = blueButtons[step1 + "-" + step2][0];
            document.getElementById("square-"+step1+"-"+step2).innerHTML = numblue;
